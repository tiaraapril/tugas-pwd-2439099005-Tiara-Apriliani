...
<body>
    <h1 class="text-center my-5">Data Sampah</h1>
        ...
        <table class="table table-striped mt-3" border="1" cellspacing="0" cellpadding="10">
            ...
            <?php foreach ($data_peserta as $data): ?>
                <tr>
                    ...
                    <td><?= $data['country'] ?></td>
                    <td><?= $data['address'] ?></td>
                    <td><a href="pulihkan.php?id=<?= $data['id'] ?>"class="btn btn-success">Pulihkan</a> | <a href="hapus_permanen.php?id=<?= $data['id'] ?>"class="btn btn-danger">Hapus Permanen</a></td>
                </tr>
            <?php endforeach ?>
        </table>
    </div>
